			...
			<div class="flex gap-2 self-start md:self-auto overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
				{#each [ { id: 'all', label: 'All Updates' }, { id: 'releases', label: 'Releases' }, { id: 'security', label: 'Security' } ] as filter}
					<button
						...
						class="px-3.5 py-1.5 rounded-lg border text-xs font-semibold capitalize transition-all cursor-pointer whitespace-nowrap {selectedCategory === filter.id ? 'bg-primary text-on-primary border-primary' : 'bg-white border-outline-variant text-on-surface-variant'}"
					>
						{filter.label}
					</button>
				{/each}
...
<span class="bg-error-container text-error px-2.5 py-0.5 rounded text-xs font-bold uppercase tracking-wider">Security</span>
								{:else}
									<span class="bg-primary/10 text-primary px-2.5 py-0.5 rounded text-xs font-bold uppercase tracking-wider">Release</span>
								{/if}
								<span class="text-xs text-outline">{item.date}</span>
							</div>
							<h2 class="font-headline-lg text-lg text-on-surface font-bold mb-3 group-hover:text-primary transition-colors">
								{item.title}
							</h2>
