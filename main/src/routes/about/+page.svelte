	...
	<div class="max-w-container-max mx-auto">
		<div class="text-center mb-12 flex flex-col items-center">
			<div class="w-16 h-16 bg-primary/10 dark:bg-primary-fixed-dim/15 rounded-full flex items-center justify-center mb-4 text-primary dark:text-primary-fixed shadow-sm">
				<span aria-hidden="true" class="material-symbols-outlined text-4xl" style="font-variation-settings: 'FILL' 1;">info</span>
			</div>
			<h1 class="font-headline-xl text-3xl md:text-5xl text-on-background dark:text-white font-extrabold tracking-tight mb-4">Dovecot Features and Highlights</h1>
			<p class="font-body-md text-body-md text-on-surface-variant dark:text-outline-variant max-w-2xl mx-auto">
				Engineered for performance, security, and open standards compliance.
			</p>
		</div>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-gutter">
			{#each features as feat}
				<div class="bg-surface-container-lowest dark:bg-inverse-surface/50 rounded-2xl p-gutter border border-outline-variant dark:border-white/5 shadow-[0px_4px_20px_rgba(26,31,43,0.02)] hover:shadow-[0px_8px_30px_rgba(26,31,43,0.06)] transition-shadow duration-300">
					...
					<h2 class="font-headline-lg text-headline-lg-mobile text-on-surface dark:text-white mb-3">{feat.title}</h2>
					<p class="font-body-md text-body-md text-on-surface-variant dark:text-outline-variant">{@html feat.desc}</p>
				</div>
			{/each}
		</div>
	</div>
