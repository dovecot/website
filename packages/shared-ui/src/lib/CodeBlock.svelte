...
<div class="space-y-2">
	...
	<div class="relative bg-[#1A1F2B] rounded-lg overflow-hidden group">
		...
		<button
			...
			class="absolute right-3 top-3 bg-white/10 hover:bg-white/20 text-white p-1.5 rounded cursor-pointer transition-colors"
			aria-label={ariaLabel}
			aria-live="polite"
		>
			<span class="material-symbols-outlined text-sm" aria-hidden="true">{copiedStates[copyId] ? 'check' : 'content_copy'}</span>
		</button>
