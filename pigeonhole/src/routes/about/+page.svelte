...
<!-- Overview Section -->
<section class="py-12 px-gutter bg-surface-container-low dark:bg-inverse-surface/40 border-b border-outline-variant/30">
	<div class="max-w-container-max mx-auto">
		<div class="text-center max-w-2xl mx-auto mb-10">
			<h2 class="font-headline-xl text-3xl text-on-background dark:text-white font-extrabold mb-3">Project Overview</h2>
			<div class="h-1 w-16 bg-primary mx-auto rounded"></div>
		</div>
		
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 font-body-md text-on-surface-variant dark:text-outline-variant leading-relaxed text-sm md:text-base">
			<div class="space-y-4">
				<p>
				...
				<p class="bg-primary/5 dark:bg-white/5 border border-primary-container/20 dark:border-white/5 p-4 rounded-xl italic">
...
<section class="py-16 px-gutter bg-surface-container-lowest dark:bg-inverse-surface/20 border-t border-outline-variant/30">
	<div class="max-w-container-max mx-auto">
		<div class="text-center mb-12 flex flex-col items-center">
			...
			<h2 class="font-headline-xl text-3xl md:text-5xl text-on-background dark:text-white font-extrabold tracking-tight mb-4">Standards & Extensions Support</h2>
		...
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
			...
			<div class="lg:col-span-8 bg-surface-container-low dark:bg-inverse-surface/40 p-6 md:p-8 rounded-2xl border border-outline-variant/40 dark:border-white/5 space-y-6">
				<div>
					<h3 class="font-headline-lg text-xl text-on-background dark:text-white font-bold mb-2">Sieve Language & RFC Extensions</h3>
					<p class="font-body-md text-sm text-on-surface-variant dark:text-outline-variant">
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
					{#each rfcExtensions as ext}
						<a 
							...
							target="_blank" 
							rel="noreferrer"
							class="flex flex-col p-3 rounded-xl bg-surface hover:bg-surface-dim dark:bg-inverse-surface/30 dark:hover:bg-white/5 border border-outline-variant/30 transition-all hover:scale-[1.01] hover:border-primary/50 group"
						>
							<div class="flex justify-between items-center">
								<span class="font-mono text-sm font-semibold text-on-surface dark:text-white group-hover:text-primary dark:group-hover:text-primary-fixed transition-colors">
									{ext.name}
								</span>
								{#if ext.version}
									<span class="text-[11px] bg-primary/10 text-primary dark:bg-primary-fixed/20 dark:text-primary-fixed px-1.5 py-0.5 rounded font-bold">{ext.version}</span>
								{/if}
							</div>
							<span class="text-[11px] text-on-surface-variant/70 dark:text-outline-variant/70 font-mono mt-1">{ext.rfc}</span>
						</a>
					{/each}
				</div>
			</div>
			...
			<div class="lg:col-span-4 space-y-8">
				...
				<div class="bg-surface-container-low dark:bg-inverse-surface/40 p-6 rounded-2xl border border-outline-variant/40 dark:border-white/5 space-y-3">
					<div class="flex items-center gap-3">
						<span aria-hidden="true" class="material-symbols-outlined text-primary text-2xl">dns</span>
						<h3 class="font-headline-lg text-lg text-on-background dark:text-white font-bold">ManageSieve</h3>
					</div>
				...
				<div class="bg-surface-container-low dark:bg-inverse-surface/40 p-6 rounded-2xl border border-outline-variant/40 dark:border-white/5 space-y-4">
					<div>
						<h3 class="font-headline-lg text-lg text-on-background dark:text-white font-bold mb-1">Dovecot Specific Extensions</h3>
						<p class="font-body-md text-xs text-on-surface-variant dark:text-outline-variant">
					...
					<div class="space-y-2">
						{#each dovecotExtensions as ext}
							<div class="p-3 bg-surface dark:bg-inverse-surface/30 rounded-xl border border-outline-variant/20 space-y-1">
								<div class="flex justify-between items-center">
									<a href={ext.link} target="_blank" rel="noreferrer" class="font-mono text-xs font-bold link-subtle">
										{ext.name}
									</a>
									<span class="text-[11px] bg-slate-100 dark:bg-white/10 text-on-surface-variant dark:text-outline-variant px-1.5 py-0.5 rounded font-mono font-bold">{ext.version}</span>
								</div>
								<p class="text-xs text-on-surface-variant dark:text-outline-variant leading-relaxed">{ext.desc}</p>
							</div>
						{/each}
					</div>
				</div>
				...
				<div class="bg-surface-container-low dark:bg-inverse-surface/40 p-6 rounded-2xl border border-outline-variant/40 dark:border-white/5 space-y-4">
					<div>
						<h3 class="font-headline-lg text-lg text-on-background dark:text-white font-bold mb-1">Under Development & Planned</h3>
						<p class="font-body-md text-xs text-on-surface-variant dark:text-outline-variant">
							Future additions and features being developed for upcoming releases.
						</p>
					</div>

					<div class="space-y-3 text-xs text-on-surface-variant dark:text-outline-variant">
						<!-- Under dev -->
						<div>
							<h4 class="font-bold text-on-background dark:text-white mb-1.5 flex items-center gap-1.5"><span class="w-1.5 h-1.5 rounded-full bg-amber-500"></span> Under Development</h4>
						...
						<div class="border-t border-outline-variant/20 pt-3">
							<h4 class="font-bold text-on-background dark:text-white mb-1.5 flex items-center gap-1.5"><span class="w-1.5 h-1.5 rounded-full bg-blue-500"></span> Planned Extensions</h4>
							<ul class="space-y-1 pl-3 list-disc">
								{#each otherExtensions.planned as item}
									<li>
										...
										{#if item.note}
											- <span class="italic text-xs text-amber-600 dark:text-amber-400">{item.note}</span>
										{/if}
									</li>
								{/each}
							</ul>
