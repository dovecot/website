...
<template>
  <div v-for="distro in props.distros">
    ...
    <p v-if="distro.keyring">
    <p>Create <code>/usr/share/keyrings/dovecot.gpg</code></p>
    <pre><code>curl <a :href="`https://repo.dovecot.org/${distro.signed_by}`">https://repo.dovecot.org/{{ distro.signed_by }}</a> | gpg --dearmor -o /usr/share/keyrings/dovecot.gpg</code></pre>
    </p>
    <p v-else>
    <p>Create <code>/etc/apt/trusted.gpg.d/dovecot.gpg</code></p>
    <pre><code>curl <a :href="`https://repo.dovecot.org/${distro.signed_by}`">https://repo.dovecot.org/{{ distro.signed_by }}</a> | gpg --dearmor -o /etc/apt/trusted.gpg.d/dovecot.gpg</code></pre>
    </p>
    <div v-if="distro.format == 'list'">
      ...
      <pre v-if="distro.dovecot_version == 'latest'"><code>deb [signed-by=/usr/share/keyrings/dovecot.gpg] <a :href="`https://repo.dovecot.org/${props.latestDir}/${family}/${distro.codename}`">https://repo.dovecot.org/{{ props.latestDir}}/{{ family }}/{{ distro.codename }}</a> {{distro.codename}} main</code></pre>
      <pre v-else><code>deb [signed-by=/usr/share/keyrings/dovecot.gpg] <a :href="`https://repo.dovecot.org/ce-${distro.dovecot_version}/${family}/${distro.codename}`">https://repo.dovecot.org/ce-{{ distro.dovecot_version }}/{{ family }}/{{ distro.codename }}</a> {{distro.codename}} main</code></pre>
    </div>
    <div v-if="distro.format == 'sources'">
      ...
      <pre><code v-if="distro.dovecot_version == 'latest'">Types: deb
...
Suites: {{ distro.codename }}
Components: main
Signed-By: /usr/share/keyrings/dovecot.gpg</code></pre>
  </div>
  </div>
  <h3>Instructions</h3>
